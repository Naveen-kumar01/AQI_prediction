...
class Model_Finder:
    ...
    def get_best_params_for_random_forest(self,train_x,train_y):
        ...
        try:
            ...
            self.param_grid = {"n_estimators": [10, 50, 100, 130], "max_depth": range(2, 6, 1), "max_features": ['auto', 'log2']}
    ...
    def get_best_params_for_xgboost(self,train_x,train_y):
        ...
        try:
            ...
            # Creating an object of the Grid Search class
            self.grid= GridSearchCV(XGBRegressor(),self.param_grid_xgboost, verbose=3,cv=5)
            # finding the best parameters
            self.grid.fit(train_x, train_y)
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            if len(test_y.unique()) == 1: #if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                ...
                self.xgboost_score = mean_squared_error(test_y, self.prediction_xgboost) # AUC for XGBoost
                self.logger_object.log(self.file_object, 'mean_squared_error for XGBoost:' + str(self.xgboost_score)) # Log AUC
            ...
            if len(test_y.unique()) == 1:#if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                ...
                self.random_forest_score = mean_squared_error(test_y, self.prediction_random_forest) # AUC for Random Forest
                self.logger_object.log(self.file_object, 'mean_squared_error for RF:' + str(self.random_forest_score))

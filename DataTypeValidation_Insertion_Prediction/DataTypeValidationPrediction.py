...
class dBOperation:
    ...
    def insertIntoTableGoodData(self,Database):
        ...
        try:
            col = ['City', 'Date', 'PM2_5', 'PM10', 'NO', 'NO2', 'NOx', 'NH3', 'CO', 'SO2', 'O3', 'Benzene', 'Toluene',
               'Xylene', 'AQI_Bucket']
        ...
        try:
            for row in real_df.itertuples():
                try:
                    conn.execute(f'''INSERT INTO Good_Raw_Data(City,Date,PM2_5,PM10,NO,NO2,NOx,NH3,CO,SO2,O3,Benzene,Toluene,Xylene,AQI_Bucket) values ('{row.City}','{row.Date}',{row.PM2_5},{row.PM10},{row.NO},{row.NO2},{row.NOx},{row.NH3},{row.CO},{row.SO2},{row.O3},{row.Benzene},{row.Toluene},{row.Xylene},'{row.AQI_Bucket}')''')
